 ...
 * `APNS_CERTIFICATE`: Path to your `Certificate.pem` file. For Heroku set to `APNS_CERT`: This file is automatically generated at the path specified in `settings.py` from the `APNS_CERTIFICATE` environmental variable by the `bin/post_compile` hook.
 * `APNS_HOST`: Address to the APNS Host. Should be one of `settings.APNS_HOST_DEV` or `settings.APNS_HOST_PROD`, depending on which APNS certificate you are using
 * `APNS_FEEDBACK_HOST`: Address to the APNS Feedback Host. Should be one of `settings.APNS_FEEDBACK_HOST_DEV` or `settings.APNS_FEEDBACK_HOST_PROD`, depending on which APNS certificate you are using
 ...
 * `GCM_API_KEY` : Your Google Cloud Messaging `Server Api Key`
 * `APNS_CERTIFICATE` : Your APNS certificate contents. This can be added via `heroku config:add APNS_CERTIFICATE=“$(cat privatekey.pem)”`
 * `APNS_USE_SANDBOX` : `True` of `False`. Will supply the appropriate values for `APNS_HOST` and `APNS_FEEDBACK_HOST`
