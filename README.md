 ...
 * `APNS_CERTIFICATE`: Path to your `Certificate.pem` file. For Heroku you don't edit this setting: This file is automatically generated at the path specified in `settings.py` from the `APNS_CERTIFICATE` environmental variable by the `bin/post_compile` hook.
