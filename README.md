 ...
 * `APNS_CERTIFICATE` (str): Path to your `Certificate.pem` file. For Heroku set to `APNS_CERT`: This file is automatically generated at the path specified in `settings.py` from the `APNS_CERTIFICATE` environmental variable by the `bin/post_compile` hook.
